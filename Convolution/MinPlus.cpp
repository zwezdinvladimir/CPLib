...
template<typename T>
bool is_convex(std::vector<T> a) {
    bool ok = true;
    for (int i = 0; i + 2 < a.size(); ++i) {
        ok &= a[i + 1] - a[i] <= a[i + 2] - a[i + 1];
    }
    return ok;
}

template<typename T>
std::vector<T> min_plus_convex_convex(std::vector<T> a, std::vector<T> b) {
    std::vector<T> c(a.size() + b.size() - 1);
    c[0] = a[0] + b[0];
    for (int i = a.size() - 1; i > 0; --i) a[i] -= a[i - 1];
    for (int i = b.size() - 1; i > 0; --i) b[i] -= b[i - 1];
    std::merge(a.begin() + 1, a.end(), b.begin() + 1, b.end(), c.begin() + 1);
    std::partial_sum(c.begin(), c.end(), c.begin());
    return c;
}
template<typename T>
std::vector<T> min_plus_convex_arbitary(std::vector<T> a, std::vector<T> b) {
    std::vector<T> c(a.size() + b.size() - 1, std::numeric_limits<T>::max());
    auto dc = [&](auto dc, int l, int r, int _l, int _r) {
        if (l > r) return;
        int m = (l + r) / 2;
        int opt = -1;
        for (int i = _l; i <= _r && i <= m; ++i) {
            if (m - i >= a.size()) continue;
            if (c[m] >= b[i] + a[m - i]) {
                c[m] = b[i] + a[m - i];
                opt = i;
            }
        }
        dc(dc, l, m - 1, _l, opt), dc(dc, m + 1, r, opt, _r);
    };
    dc(dc, 0, c.size() - 1, 0, b.size() - 1);
    return c;
}
template<typename T>
std::vector<T> min_plus_stupid(std::vector<T> a, std::vector<T> b) {
    std::vector<T> c(a.size() + b.size() - 1, std::numeric_limits<T>::max());
...
template<typename T>
std::vector<T> min_plus(std::vector<T> a, std::vector<T> b) {
    bool convA = is_convex(a), convB = is_convex(b);
    if (convA && convB) return min_plus_convex_convex(a, b);
    if (convA) return min_plus_convex_arbitary(a, b);
    if (convB) return min_plus_convex_arbitary(b, a);
    return min_plus_stupid(a, b);
}
